  ...
  useEffect(() => {
    ...
    const doSearch = async () => {
      ...
      const [catRes, localRes, propRes] = await Promise.all([
        supabase.from('guia_categorias').select('id, nome, slug, icone, descricao').ilike('nome', term).limit(10),
        supabase.from('locais').select('id, nome, slug, categoria, imagem_destaque, endereco').eq('ativo', true).ilike('nome', term).order('ordem').limit(20),
        supabase.from('properties').select('id, title, slug, location, thumbnail_url, image_url, transaction_type, price_formatted').eq('status', 'active').ilike('title', term).limit(20),
      ]);

      if (cancelled) return;

      const items: SearchResult[] = [];
      (catRes.data ?? []).forEach((c: any) => items.push({ id: c.id, title: c.nome, subtitle: c.descricao, url: `/guia/categoria/${c.slug}`, type: 'categoria', icon: c.icone }));
      (localRes.data ?? []).forEach((l: any) => items.push({ id: l.id, title: l.nome, subtitle: l.endereco || l.categoria, url: `/locais/${l.slug}`, type: 'local', image: l.imagem_destaque }));
      (propRes.data ?? []).forEach((p: any) => items.push({ id: p.id, title: p.title || 'Imóvel', subtitle: [p.location, p.price_formatted].filter(Boolean).join(' • '), url: `/imoveis/${p.transaction_type === 'temporada' ? 'temporada' : 'venda'}/${p.slug || p.id}`, type: 'imovel', image: p.thumbnail_url || p.image_url }));
