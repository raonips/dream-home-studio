...
const AdminSiteConfig = () => {
  ...
  const handleLogoUpload = async (
    ...
    try {
      const safePath = `site/${Date.now()}-${file.name.replace(/[^a-z0-9.]/gi, '-')}`;
  ...
  const handleHeroBannerUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Generate 2 versions: Desktop (1920px) and Mobile (800px)
      ...
      const files = [
        ...
        const f = new File([blob], `${ts}-${baseName}-${suffix}.webp`, { type: 'image/webp' });
  ...
  return (
    <>
      <Helmet><title>Configurações do Site | Admin</title></Helmet>
      <div className="space-y-6 max-w-4xl">
        <div>
          <h1 className="text-2xl font-bold font-display text-foreground">Configurações do Site</h1>
          <p className="text-sm text-muted-foreground mt-1">
            Gerencie a identidade visual, conteúdo da Home e proteção de imagens.
          </p>
        </div>
          ...
          <TabsContent value="geral" className="space-y-6 mt-6 max-w-2xl">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Exibida no cabeçalho do site. Recomendado: PNG transparente, máx. 250×80px.</CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="flex items-center gap-4">
                  {form.logo_url ? (
                    <div className="relative">
                      <img src={form.logo_url} alt="Logo" className="h-14 max-w-[200px] object-contain rounded border border-border p-1 bg-background" />
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Imagem de fundo da seção principal da Home. Recomendado: 1920×1080px.
                  <br />
                  <span className="text-xs font-medium text-primary">O sistema gera automaticamente versões Desktop e Mobile em WebP otimizado.</span>
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                {form.hero_bg_desktop || form.hero_image_url ? (
                  ...
                  <div className="border-2 border-dashed border-border rounded-lg p-8 text-center cursor-pointer hover:border-primary/50 transition-colors" onClick={() => heroRef.current?.click()}>
                    <ImageIcon className="h-10 w-10 mx-auto text-muted-foreground mb-3" />
                    <p className="text-sm text-muted-foreground">Clique para enviar o banner da Home</p>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Título e subtítulo exibidos sobre o banner principal.</CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="space-y-2">
                  <Label>Título Principal (H1)</Label>
                  <Input value={form.hero_title} onChange={set('hero_title')} placeholder="Comprar Casa em Barra do Jacuípe BA" />
                </div>
                <div className="space-y-2">
                  <Label>Subtítulo</Label>
                  <Input value={form.hero_subtitle} onChange={set('hero_subtitle')} placeholder="Encontre a casa dos seus sonhos no Litoral Norte baiano." />
                </div>
              </CardContent>
            </Card>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Número do WhatsApp (com DDD e código do país) e link do Instagram.</CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="space-y-2">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Escolha qual serviço de mapa será usado nas páginas de imóveis e condomínios.</CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="space-y-2">
                ...
                {form.map_provider === 'google' && (
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-muted-foreground">
                      Obtenha em <a href="https://console.cloud.google.com/apis/credentials" target="_blank" rel="noopener noreferrer" className="text-primary hover:underline">Google Cloud Console</a>. Sem chave, o sistema usará OpenStreetMap.
                    </p>
                  </div>
                )}
              </CardContent>
