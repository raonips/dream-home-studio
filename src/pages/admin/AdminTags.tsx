...
const AdminTags = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    let error;
    if (editing) {
      ({ error } = await supabase.from('tags').update(payload).eq('id', editing.id));
    } else {
      ({ error } = await supabase.from('tags').insert(payload));
    }
    setSaving(false);
    if (error) {
  ...
  return (
    <>
      <Helmet><title>Características / Tags | Admin</title></Helmet>
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold font-display text-foreground">Características / Tags</h1>
        ...
        <Card>
          <CardContent className="p-0">
            {loading ? (
              ...
              <div className="text-center py-12 text-muted-foreground">Nenhuma tag cadastrada.</div>
            ) : (
              <Table>
                <TableHeader>
                  <TableRow>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>Preencha o nome e a descrição SEO da característica.</DialogDescription>
          </DialogHeader>
